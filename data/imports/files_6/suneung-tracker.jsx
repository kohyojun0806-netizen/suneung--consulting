...
const LANDING_STORYLINES = [
  {
    headline: '실제 합격·실패 데이터 기반',
    detail: '공식 페이지, 수강 후기, 강사 게시판, Q&A 데이터를 교차 검증합니다.',
  },
  {
    headline: '수능 수학 코칭 인사이트',
    detail: '등급과 시기별 우선순위를 분리해 지금 해야 할 행동만 남깁니다.',
  },
  {
    headline: '수준별 로드맵 설계',
    detail: '노베이스부터 100점권까지, 분기 전략이 명확한 하나의 로드맵을 제공합니다.',
  },
];
...
const LEVEL_LENS = [
  ...
  { id: '100', label: '100점권', cue: '만점 루틴 유지' },
];
  ...
  return (
    <div className={`landing${visible ? ' landing--visible' : ''}${exiting ? ' landing--exit' : ''}`}
      ...
      <div className="landing__badge">
        ...
        <span>오로지 실제 성공·실패 데이터를 기반으로 한 정시 수학 컨설팅</span>
        <span style={{ opacity: 0.62, fontSize: '11px' }}>수능수학전문가의 인사이트</span>
      </div>
      ...
      <div className="landing__hero">
        ...
        <section className="landing__story" aria-label="등급대별 전략 렌즈">
          ...
          {[
            ...
            <div key={label} className="landing__ladder-row">
              <span className="landing__ladder-label">{label}</span>
              <div className="landing__ladder-track" aria-hidden="true">
                <div className="landing__ladder-fill" style={{ width: `${pct}%` }} />
              </div>
              <span className="landing__ladder-cue">{cue}</span>
            </div>
          ))}

          {LANDING_STORYLINES.map((item) => (
            <article key={item.headline} className="landing__story-card">
